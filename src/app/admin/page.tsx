      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-6 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 p-8 text-white lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-amber-400">Admin dashboard</p>
              <h1 className="mt-3 text-3xl font-semibold">Applicants overview</h1>
              <p className="mt-3 max-w-2xl text-sm text-slate-300">
                Review submissions, move applications through the pipeline, and keep the campaign team aligned with a clear snapshot of progress.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link href="/admin/applicants" className="rounded-full bg-white px-5 py-3 font-medium text-slate-900">
                Review applicants
              </Link>
              <Link href="/admin/applicants/export" className="rounded-full border border-white/30 px-5 py-3 font-medium text-white">
                Export CSV
              </Link>
            </div>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {stats.map((stat) => {
            ...
            return (
              <div key={stat.label} className="rounded-[1.25rem] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200">
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {(applications ?? []).slice(0, 6).map((application: { id: string; first_name: string; surname: string; phone: string; status: string; created_at: string }) => (
                  <tr key={application.id}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-3 py-1 text-xs font-medium uppercase ${statusClass(application.status)}`}>
                        {application.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-600">{new Date(application.created_at).toLocaleDateString()}</td>
                    <td className="px-4 py-3">
