...
export default async function ApplicantDetailPage({
  ...
  const [{ data: ward }, { data: pollingUnit }] = await Promise.all([
    supabase
      .from("wards")
      .select("name")
      .eq("id", application.ward_id)
      .single(),

    supabase
      .from("polling_units")
      .select("name")
      .eq("id", application.polling_unit_id)
      .single(),
  ]);
          ...
          <div className="rounded-[1.5rem] border border-slate-200 bg-white p-8 shadow-sm">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-[1.1rem] border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-2 font-semibold text-slate-900">
                  {application.has_smartphone ? "Yes" : "No"}
                </p>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3 text-sm text-slate-600">
                <a
                  href={application.membership_card_url}
                  target="_blank"
                  rel="noreferrer"
                  className="block rounded-full border border-slate-200 px-4 py-3 text-center font-medium text-slate-700"
                >
                  View membership card
                </a>

                <a
                  href={application.passport_url}
                  target="_blank"
                  rel="noreferrer"
                  className="block rounded-full border border-slate-200 px-4 py-3 text-center font-medium text-slate-700"
                >
